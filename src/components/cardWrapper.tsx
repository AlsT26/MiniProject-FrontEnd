...
export default function Wrapper() {
  ...
  if (!dataEvents || dataEvents.length === 0) {
    return <div className="text-center my-10">No events available at the moment.</div>;
  }

  return (
    <div className="flex flex-wrap gap-4 justify-center my-12 px-4">
      {dataEvents.map((event, idx) => (
        <Card
          key={idx}
          title={event.title}
          thumbnail={event.thumbnail}
          avatar={event.promotor.avatar || "https://res.cloudinary.com/dn6uglajh/image/upload/v1733990935/blank-image_yfczs3.jpg"}
